...
def compare_raters(directory):
    ...
    column and a 'label' column. The values of the label column must only be
    'neu', 'pos' or 'neg'.
    ...
    for number, fileitem in enumerate(files, 1):
        ...
        if sorted(temp_df['label'].unique()) != ["neg", "neu", "pos"]:
            print(
                f"ERROR in {fileitem}: The 'label' column can only contain the values 'neg','neu'','pos' in smal caps.")
            return None
    ...
    if no_agreement > 0:
        print(
            f"No agreement was found for {no_agreement} observations. Please change this before calling the sentiment_analysis() fucntion.")
